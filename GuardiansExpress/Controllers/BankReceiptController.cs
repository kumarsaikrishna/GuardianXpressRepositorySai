...
namespace GuardiansExpress.Controllers
{
    public class BankReceiptController : Controller
    {
        ...
        private readonly IBankReceiptService _bankReceiptService;

        public BankReceiptController(MyDbContext context, IBankReceiptService bankReceiptService)
        {
            _context = context;
            _bankReceiptService = bankReceiptService;
        }

        public IActionResult BankReceiptIndex()
        {
        ...
        public IActionResult Search()
        {
            ...
            var model = _bankReceiptService.GetBankReceiptRegisterDetails(
                string.IsNullOrEmpty(branchId) ? null : int.Parse(branchId),
                fromDate,toDate);

            TempData["BankReceiptRegisterData"] = JsonConvert.SerializeObject(model);
            return View("BankReceiptIndex", model);
        }

        public IActionResult ExportToPdf()
        {
            if (TempData["BankReceiptRegisterData"] != null)
            {
                var data = JsonConvert.DeserializeObject<List<VoucherDto>>(TempData["BankReceiptRegisterData"].ToString());
                using (var stream = new MemoryStream())
                {
                    ...
                    var title = new Paragraph("Bank Receipt Register Report", titleFont)
                    {
                    ...
                    return File(content, "application/pdf", "BankReceiptRegisterReport.pdf");
                }
            }

            return RedirectToAction("BankReceiptIndex");
        }
        ...
        public IActionResult ExportToExcel()
        {
            if (TempData["BankReceiptRegisterData"] != null)
            {
                ...
                {
                    var worksheet = package.Workbook.Worksheets.Add("Bank Receipt Register Report");

                    worksheet.Cells[1, 1].Value = "Bank Receipt Register Report";
