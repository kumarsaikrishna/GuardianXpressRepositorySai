...
namespace GuardiansExpress.Controllers
{
    public class BankPaymentController : Controller
    {
        ...
        public BankPaymentController(MyDbContext context, IBankPaymentService bankPaymentService)
        {
            _context = context;
            _bankPaymentService = bankPaymentService;
        }

        public IActionResult BankPaymentIndex()
        {
        ...
        {
            if (TempData["BankPaymentRegisterData"] != null)
            {
                var data = JsonConvert.DeserializeObject<List<VoucherDto>>(TempData["BankPaymentRegisterData"].ToString());
                using (var stream = new MemoryStream())
                {
                    ...
                    var title = new Paragraph("Bank Payment Register Report", titleFont)
                    {
                    ...
                    return File(content, "application/pdf", "BankPaymentRegisterReport.pdf");
                }
            }
        ...
        public IActionResult ExportToExcel()
        {
            if (TempData["BankPaymentRegisterData"] != null)
            {
                ...
                {
                    var worksheet = package.Workbook.Worksheets.Add("Bank Payment Register Report");

                    worksheet.Cells[1, 1].Value = "Bank Payment Register Report";
