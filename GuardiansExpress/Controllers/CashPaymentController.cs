...
namespace GuardiansExpress.Controllers
{
    public class CashPaymentController : Controller
    {
        ...
        public CashPaymentController(MyDbContext context, ICashPaymentService cashPaymentService)
        {
            _context = context;
            _cashPaymentService = cashPaymentService;
        }

        public IActionResult CashPaymentIndex()
        {
        ...
        public IActionResult Search()
        {
            ...
            var model = _cashPaymentService.GetCashPaymentRegisterDetails(
                string.IsNullOrEmpty(branchId) ? null : int.Parse(branchId),
                fromDate,
                toDate);

            TempData["CashPaymentRegisterData"] = JsonConvert.SerializeObject(model);
            return View("CashPaymentIndex", model);
        }

        public IActionResult ExportToPdf()
        {
            if (TempData["CashPaymentRegisterData"] != null)
            {
                var data = JsonConvert.DeserializeObject<List<VoucherDto>>(TempData["CashPaymentRegisterData"].ToString());
                using (var stream = new MemoryStream())
                {
                    ...
                    var title = new Paragraph("Cash Payment Register Report", titleFont)
                    {
                    ...
                    return File(content, "application/pdf", "CashPaymentRegisterReport.pdf");
                }
            }

            return RedirectToAction("CashPaymentIndex");
        }
        ...
        public IActionResult ExportToExcel()
        {
            if (TempData["CashPaymentRegisterData"] != null)
            {
                ...
                {
                    var worksheet = package.Workbook.Worksheets.Add("Cash Payment Register Report");

                    worksheet.Cells[1, 1].Value = "Cash Payment Register Report";
