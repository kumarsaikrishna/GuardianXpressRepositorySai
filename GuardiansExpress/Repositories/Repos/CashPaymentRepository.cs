...
{
    public class CashPaymentRepository : ICashPaymentRepository
    {
        private readonly MyDbContext _context;

        public CashPaymentRepository(MyDbContext context)
        {
            _context = context;
        }
        ...
        public List<VoucherDto> GetCashPaymentRegisterDetails(int? branchId, string fromDate, string toDate)
        {
            var query = from voucher in _context.Vouchers
                        ...
                        where voucher.VoucherType == "Cash Payment" // Filter only Cash Receipt vouchers
